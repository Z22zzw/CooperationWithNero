...
// 处理API错误
if (error.value) {
  console.error('初始化数据失败:', error.value)
  // 生产环境可考虑重试逻辑
}
watchEffect(() => {
  if (data.value?.menu?.projects) {
    cards.value = data.value.menu.projects
  }
})
onMounted(() => {
  if (cards.value.length === 0 && !pending.value) {
    console.warn('客户端检测到空数据，重新获取')
    refresh()
  }
})
